		...
		while(true) {
			...
			switch(choice) {
				case 1 : 
					...
					break;
				case 2 : 
					member = this.inputLoginInfo();	// 위에서 Member member 선언했기 때문에 member만 써도 됨
					// 입력한 ID와 PW이 DB에 있는가?
				...
				case 4 : 
					...
					// 밑에건 내가 한거 .. 위에는 check 넣어서
//					result = mController.removeMember(memberId);
//					if(result > 0) {
//						printMessage("회원탈퇴 성공!");
//					} else {
//						printMessage("회원탈퇴 실패!");
//					}
					break;
				case 5 : break;
				case 6 : break;
				case 9 : 
					printMessage("프로그램 종료");	// this 안 적어도 됨
					break finish;
			}
		}
	}
	...
	private Member modifyMember() {
		...
		// 이것들을 필드로 가지는 member 생성자 새로 생성해야한다. 
		return member;
	}
